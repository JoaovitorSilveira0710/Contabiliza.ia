...
class NotaFiscal(Base):
    ...
    # 🔥 CORREÇÃO: Remover back_populates problemáticos
    cliente = relationship("Cliente", backref="notas_fiscais")
    
    # 🔥 CORREÇÃO: Comentar relacionamentos que dependem de outros modelos
    ...
    def dias_desde_emissao(self):
        hoje = datetime.now()
        return (hoje - self.data_emissao).days if self.data_emissao else None
...
class ItemNotaFiscal(Base):
    ...
    # 🔥 CORREÇÃO: Relacionamento interno pode manter
...
class EventoNotaFiscal(Base):
    ...
    __table_args__ = (
        CheckConstraint("tipo_evento IN ('autorizacao', 'cancelamento', 'carta_correcao', 'inutilizacao', 'epce')", name='check_tipo_evento'),
    )
    ...
    def to_json(self):
        ...
        def json_serializer(obj):
            ...
            if isinstance(obj, _uuid.UUID):
                return str(obj)
            return str(obj)  # 🔥 CORREÇÃO: str() em vez de obj
        
        data = self.to_dict()
        return json.dumps(data, default=json_serializer, ensure_ascii=False)

# Aplicar mixin aos models
for model_class in [NotaFiscal, ItemNotaFiscal, EventoNotaFiscal]:
